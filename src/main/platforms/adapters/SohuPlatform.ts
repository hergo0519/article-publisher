...
export class SohuPlatform implements PlatformAdapter {
    ...
    fields: [
      {
        ...
        type: 'textarea',
        required: true,
        placeholder: '请粘贴搜狐号的 Cookie',
        help: '登录搜狐号后，在浏览器开发者工具中复制 Cookie',
      },
    ],
    features: {
      supportHTML: true,
    ...
    try {
      ...
      const result = await new Promise<any>((resolve, reject) => {
        const req = https.request(
          'https://mp.sohu.com/v2/article/save',
          {
            method: 'POST',
            headers: {
  ...
  async testConnection(credentials: any): Promise<{ success: boolean; message?: string }> {
    try {
      ...
      return new Promise((resolve) => {
        https.get('https://mp.sohu.com/v2/user/info', {
          headers: {
            'Cookie': cookie,
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
          },
